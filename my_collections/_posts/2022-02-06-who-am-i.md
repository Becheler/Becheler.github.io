...
spatial dynamics of a species over thousands of years :last_quarter_moon_with_face: :kick_scooter:

The problem is, until recently we did not really have an iDDC engine :see_no_evil: We had some blueprints and general directions from previous research, and a 20 years old blackbox simulator: [Splatche](https://www.splatche.com/splatche3). Even if it has been super useful, the fact this software is not open source put an end to many attempt to push the field further.
...
:muscle: So I decided to build these tools and components :man_factory_worker: It was a fun experience, because you end up trying to convince people they need to buy your bricks if they are to build a nice house, when everybody is used to live in a rock-cut cave! :building_construction:
